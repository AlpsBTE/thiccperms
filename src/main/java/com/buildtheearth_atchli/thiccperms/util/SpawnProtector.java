...
public class SpawnProtector {
    ...
    public void interactRightClickBlock(PlayerInteractEvent.RightClickBlock evt) {
        if (!com.buildtheearth_atchli.thiccperms.ThutPerms.allCommandUse || evt.getWorld().isRemote || FMLCommonHandler.instance().getSide() == Side.CLIENT)
            return;
    ...
    private boolean canEditSpawn(EntityPlayer player) {
        return com.buildtheearth_atchli.thiccperms.GroupManager.get_instance().hasPermission(player.getUniqueID(), "thutperms.editspawn");
    }
