            ...
            for (Class<?> candidateClass : foundClasses) {
                if (CommandBase.class.isAssignableFrom(candidateClass) && candidateClass.getEnclosingClass() == null) {
                    ...
                    try {
                        ...
                        com.buildtheearth_atchli.thiccperms.ThutPerms.logger.log(Level.WARNING, "Error with " + candidateClass, e);
                    }
                    if (move != null && move.getName() != null) {
                        event.registerServerCommand(move);
                    } else {
                        com.buildtheearth_atchli.thiccperms.ThutPerms.logger.log(Level.WARNING, candidateClass + " is not completed.");
                    }
                }
            }
        } catch (Exception e) {
            com.buildtheearth_atchli.thiccperms.ThutPerms.logger.log(Level.WARNING, "Error finding commands", e);
        }
    }
